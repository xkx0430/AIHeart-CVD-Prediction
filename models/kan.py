...
class KAN(nn.Module):
    def __init__(
        ...
        self.cat_embeds = nn.Embedding(sum(num_unique_categories), dim)
        self.cat_offset = torch.tensor(list(num_unique_categories)).cumsum(dim=-1)
        self.cat_offset = rearrange(self.cat_offset, 'n -> () 1 n')
        self.weights = nn.Parameter(torch.randn(dim, num_con))
        self.biases = nn.Parameter(torch.randn(dim, num_con))
    ...
    def forward(self, x):
        xs = []
        x_cat = x[:, :self.num_cat] + self.cat_offset.to(x.device)
        print(x_cat.shape)
        x_cat = self.cat_embeds(x_cat.to(torch.int))
        print(x_cat.shape)
        xs.append(x_cat)
        x_con = rearrange(x[:, self.num_cat:], 'b n -> b 1 n')
        x_con = x_con * self.weights + self.biases
        xs.append(x_con)
        print(x_con.shape)
